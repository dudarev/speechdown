...
class FileOutputAdapter(OutputPort):
    ...
    def output_transcription_results(
        self, transcription_results: list[TranscriptionResult], path: Path | None = None, timestamp: datetime.datetime | None = None
    ) -> None:
        ...
        # Group transcriptions by date (using current date as default)
        date_to_transcriptions: Dict[datetime.date, List[TranscriptionResult]] = {}  # Using date as string
        current_date = (timestamp or datetime.datetime.now()).date()
        
        for result in transcription_results:
            # Use current date for all transcriptions in this implementation
            # Future implementations could extract date from audio file metadata
        ...
        for transcription_date, results in date_to_transcriptions.items():
            ...
            self._write_to_file(file_path, results, timestamp=timestamp)
    ...
    def _write_to_file(self, file_path: Path, results: list[TranscriptionResult], timestamp: datetime.datetime | None = None) -> None:
        ...
        new_transcriptions_markdown = self._format_results_to_markdown_sections(results, timestamp=timestamp)
        
        # Use MarkdownMerger to merge
        merged_content = self.markdown_merger.merge_content(existing_content, new_transcriptions_markdown)
        
        file_path.write_text(merged_content)
        logger.info(f"Transcription results written to {file_path}")

    def _format_results_to_markdown_sections(self, transcription_results: list[TranscriptionResult], timestamp: datetime.datetime | None = None) -> str:
        """
        ...
        result_tuples = []
        for result in transcription_results:
            # Try to get a per-result timestamp if available, else use batch timestamp
            result_timestamp = getattr(result, 'timestamp', None) or timestamp or datetime.datetime.now()
