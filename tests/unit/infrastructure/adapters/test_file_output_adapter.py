...
def test_format_results_to_markdown_sections_new_content(sample_transcription):
    adapter = FileOutputAdapter(Mock())
    results = [sample_transcription]
    current_time_str = "2025-05-08 14:30:00"
    frozen_datetime = datetime.strptime(current_time_str, "%Y-%m-%d %H:%M:%S")
    # Call the new method with explicit timestamp
    formatted_markdown = adapter._format_results_to_markdown_sections(results, timestamp=frozen_datetime)
    expected_header = f"## {current_time_str}"
